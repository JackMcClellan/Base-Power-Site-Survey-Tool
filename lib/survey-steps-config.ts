// Unified survey step configuration - Backend source of truth
// This file contains ALL step data including AI prompts and structured fields

export interface StepAIConfig {
  userPrompt: string
  structuredFields?: Record<string, string>
  isDataExtraction?: boolean
}

export interface StepDataEntryConfig {
...
export interface SurveyStepComplete {
  id: number
  title: string
  description: string
  instructions: string
  tips: string[]
  stepType?: 'camera' | 'data-entry' | 'guide'
  skippable?: boolean
  cameraConfig?: Record<string, never>
  dataEntryConfig?: StepDataEntryConfig
  guideConfig?: StepGuideConfig
  // Backend-only fields
  aiConfig?: StepAIConfig
}

// Frontend step definition (without AI config)
export interface SurveyStepFrontend {
  id: number
  title: string
  description: string
  instructions: string
...
// Complete survey steps configuration (Backend only)
export const SURVEY_STEPS_COMPLETE: SurveyStepComplete[] = [
  // Guide to meter area
  {
    id: 0.5,
...
// Helper function to get frontend-safe step data
export function getStepForFrontend(stepId: number): SurveyStepFrontend | null {
  const step = SURVEY_STEPS_COMPLETE.find(s => s.id === stepId)
  if (!step) return null

  // Return step without aiConfig
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const { aiConfig, ...frontendStep } = step
  return frontendStep
}

// Helper function to get all frontend steps
export function getAllStepsForFrontend(): SurveyStepFrontend[] {
  return SURVEY_STEPS_COMPLETE.map(step => {
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const { aiConfig, ...frontendStep } = step
    return frontendStep
  })
...
// Helper function to get AI config for a step (backend only)
export function getStepAIConfig(stepId: number): StepAIConfig | null {
  const step = SURVEY_STEPS_COMPLETE.find(s => s.id === stepId)
  return step?.aiConfig || null
}

// Helper function to check if a step requires data extraction
export function isDataExtractionStep(stepId: number): boolean {
  const config = getStepAIConfig(stepId)
  return config?.isDataExtraction === true
}

// Get step sequence for navigation
export function getStepSequence(): number[] {
  return SURVEY_STEPS_COMPLETE.map(step => step.id).sort((a, b) => a - b)
}

// Get the review step ID
export function getReviewStepId(): number {
  // Review step is defined as step 13
  return 13
}

// Get next step ID
export function getNextStepId(currentStepId: number): number {
  ...
  // If we're at the last step or step not found, go to review
  return getReviewStepId()
}

// Export frontend-safe steps as a constant for static imports
// This allows the frontend to import directly without API calls
export const SURVEY_STEPS_FRONTEND = getAllStepsForFrontend()

// Export camera steps only (filtering out guide steps)
export const CAMERA_STEPS_FRONTEND = SURVEY_STEPS_FRONTEND.filter(step => step.stepType !== 'guide') 